...
/**
 * Target Channel & Pre-Release Resolver:
 * Always targets 'latest.yml' (the single standard manifest produced by electron-builder).
 * When pre-releases are enabled, inspects GitHub to determine whether the newest release
 * is stable or a pre-release, adjusting allowPrerelease so electron-updater doesn't skip
 * stable releases.
 */
export async function resolveTargetChannel(allow?: boolean): Promise<void> {
  ...
  // Always use the standard 'latest.yml' generated by electron-builder
  autoUpdater.channel = 'latest';
  autoUpdater.allowPrerelease = allowPrerelease;
  autoUpdater.allowDowngrade = !allowPrerelease && isCurrentPrerelease;

  if (!allowPrerelease) {
    console.log('[Purrsonica Updater] Pre-release disabled: Targeting "latest.yml" (allowDowngrade:', autoUpdater.allowDowngrade, ')');
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (validReleases.length > 0) {
        ...
        // If the newest release is STABLE (no pre-release suffix like -rc or -beta):
        // We disengage allowPrerelease so electron-updater's GitHubProvider fetches latest.yml
        // without getting stuck in pre-release channel filtering.
        if (!prereleaseComponents || prereleaseComponents.length === 0) {
          console.log(`[Purrsonica Updater] Target release is stable (${targetTag}). Targeting latest.yml with allowPrerelease: false.`);
          autoUpdater.allowPrerelease = false;
        } else {
          console.log(`[Purrsonica Updater] Target release is pre-release (${targetTag}). Targeting latest.yml with allowPrerelease: true.`);
